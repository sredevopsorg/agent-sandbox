...
2.  **AI-Assisted Code Reviews**:
    *   Do **NOT** directly apply code suggestions from CodeRabbit (or any AI review bot) via the GitHub UI. Doing so adds the AI bot as a co-author to the commit. Since AI bots cannot sign the Kubernetes CLA, this will cause the CLA check to fail and block the PR. Manual review and application of suggestions are required.
    *   Once the initial AI review (e.g., by CodeRabbit) is complete, ensure all review comments are addressed and resolved so the PR is ready for maintainers.
